...
class JogadorIA(Jogador):
        ...
        for l in range(0, 3):
            for c in range(0, 3):
                if self.matriz[l][c] == Tabuleiro.DESCONHECIDO:
                    ...
                    # Tenta a jogada para o oponente
    ...
    def conta_sequencias(self, tipo: int) -> int:
        ...
        for col in range(0, 3):
            coluna = [self.matriz[row][col] for row in range(0, 3)]
            if coluna.count(tipo) == 2 and coluna.count(Tabuleiro.DESCONHECIDO) == 1:
                sequencias += 1
        # Verifica diagonais
        diagonal1 = [self.matriz[i][i] for i in range(0, 3)]
        diagonal2 = [self.matriz[i][2 - i] for i in range(0, 3)]
